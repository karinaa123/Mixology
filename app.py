...
def categorize_bottles(brands, api_key):
    ...
    try:
        msg = client.messages.create(
            model="claude-sonnet-4-20250514",  # Ensure you use a valid model name you have access to
            max_tokens=1000,
...
def generate_recipe(inventory, mixers, prefs, api_key):
    ...
    * (Example: "No Shaker? Use a jar with a tight lid.")
    ...
    with client.messages.stream(
            max_tokens=1024,
            messages=[{"role": "user", "content": prompt}],
            model="claude-3-5-sonnet-20240620",  # Updated to a widely available model version
    ) as stream:
    ...
    for i, flavor in enumerate(flavors):
        ...
        if col.button(flavor, use_container_width=True):
            ...
            st.session_state.messages.append({"role": "assistant",
                                              "content": "Got it. Let's see what bottles you have working with. Upload a photo! 📸"})
        ...
        if not api_key:
            ...
            with st.spinner("Scanning your bar..."):
                ...
                for uploaded_file in uploaded_files:
                    ...
                    results = model(image, conf=0.15)  # Lower confidence to catch more bottles
                    for r in results:
                        for box in r.boxes:
